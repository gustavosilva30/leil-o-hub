...
export const extrairDadosLeiloesMS = async () => {
    ...
    try {
        ...
        for (const linkCompleto of linksDeLotes) {
            ...
            try {
                await sleep(600); // Pausa leve para não ser bloqueado
                ...
                // --- BUSCA AVANÇADA DE DATAS ---
                // Pegamos o texto bruto onde as datas costumam aparecer
                const sectionDatas = $lote(".infos-leilao, .datas-leilao, body").text().toUpperCase();
                
                // Regex para capturar data e hora (Abertura e Encerramento)
            ...
            // Validação de Sucata Aproveitável
        ...
        if (veiculosEncontrados.length > 0) {
            await axios.post(WEBHOOK_N8N, { lotes: veiculosEncontrados });
            console.log(`🚀 [LeiloesMS] ${veiculosEncontrados.length} lotes enviados para o CRM da Dourados AP!`);
        } else {

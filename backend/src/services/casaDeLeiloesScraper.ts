...
const URL_BUSCA   = "https://www.casadeleiloes.com.br/busca.aspx?p=sucata";
const PREFIXO     = "https://www.casadeleiloes.com.br";
...
export const extrairDadosCasaDeLeiloes = async (): Promise<AuctionLot[]> => {
    console.log("🔱 [CasaDeLeiloes] Iniciando busca de lotes...");
    ...
    try {
        ...
        console.log(`📡 [CasaDeLeiloes] ${linksDeLotes.size} links de lotes identificados.`);
        ...
        for (const linkCompleto of linksDeLotes) {
            ...
            try {
                ...
                console.error(`❌ [CasaDeLeiloes] Erro no lote ${numeroLoteBruto}: ${linkCompleto}`);
            }
        ...
        if (veiculosEncontrados.length > 0) {
            console.log(`🚀 [CasaDeLeiloes] ${veiculosEncontrados.length} lotes novos encontrados.`);
        } else {
            console.log("⚠️ [CasaDeLeiloes] Nenhum lote novo processado.");
        }
    } catch (error) {
        console.error("🚨 [CasaDeLeiloes] Erro fatal no Scraper:", error);
    }

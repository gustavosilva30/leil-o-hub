...
const URL_BUSCA   = "https://www.reginaaudeleiloes.com.br/busca.aspx?p=sucata";
const PREFIXO     = "https://www.reginaaudeleiloes.com.br";
...
export const extrairDadosReginaAude = async (): Promise<AuctionLot[]> => {
    console.log("🔱 [ReginaAude] Iniciando busca de lotes...");
    ...
    try {
        ...
        console.log(`📡 [ReginaAude] ${linksDeLotes.size} links de lotes identificados.`);
        ...
        for (const linkCompleto of linksDeLotes) {
            ...
            try {
                ...
                console.error(`❌ [ReginaAude] Erro no lote ${numeroLoteBruto}: ${linkCompleto}`);
            }
        ...
        if (veiculosEncontrados.length > 0) {
            console.log(`🚀 [ReginaAude] ${veiculosEncontrados.length} lotes novos encontrados.`);
        } else {
            console.log("⚠️ [ReginaAude] Nenhum lote novo processado.");
        }
    } catch (error) {
        console.error("🚨 [ReginaAude] Erro fatal no Scraper:", error);
    }
